...
class MedicalCrew:
    ...
    def medical_assistant(self) -> Agent:
        return Agent(config=self.agents_config["medical_assistant"], max_iter=2, allow_delegation=False, verbose=True)

    @agent
    def csv_agent(self) -> Agent:
        return Agent(
            config=self.agents_config["csv_agent"],
            tools=[self.medical_condition_reference, self.suggested_doctor, self.timeslot],
            verbose=True,
    ...
    def crew(self) -> Crew:
        return Crew(
            ...
            tasks=[self.get_medical_reference_task(), self.classify_illness_task(), self.get_suggested_doctor(), self.get_doctor_timeslot_task()],
